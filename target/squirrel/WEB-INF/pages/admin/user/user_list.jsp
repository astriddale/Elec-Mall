				...
				<tbody>
					<c:forEach items="${userGrid.rows}" var="item">
						<tr>
							...
							<td>${item.qq}</td>
							<td>${item.createAt}</td>
							<td><a href="../upload/${item.phone}01.jpg">学生证</a> </td>
							<td><a href="../upload/${item.phone}02.jpg">学生证2</a></td>
							<td>
							...
							<button type="button" class="btn btn-info btn-sm"  onclick="doEdit(${item.id})" >修改</button>
								<c:if test="${item.sh != 1}">
							<button type="button" class="btn btn-info btn-sm"  onclick="doSh(${item.id})">审核通过</button>
								</c:if>
						</tr>
					</c:forEach>
				</tbody>
...
<script type="text/javascript">
		...
		/* 审核通过 */
		function doSh(itemId){
			$.ajax({
				url:'<%=basePath%>admin/sh?itemId=' + itemId,
				type:'GET',
				success:function(json){
					alert(json.msg);
					//.reload();
